...
from flwr.common import (
    ...
    weights_to_parameters, bytes_to_ndarray,
)
...
class MyClient(ABC):
    ...
    @abstractmethod
    def fit(
        self, model_parameters: List[List[np.ndarray]], config: Dict[str, Scalar]
    ) -> Tuple[List[np.ndarray], int, Dict[str, Scalar]]:
...
def has_get_properties(client: MyClient) -> bool:
    """Check if NumPyClient implements get_properties."""
    return type(client).get_properties != MyClient.get_properties


class MyClientWrapper(Client):
    """Wrapper which translates between Client and NumPyClient."""

    def __init__(self, numpy_client: MyClient) -> None:
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        # Deconstruct FitIns
        log(INFO, f"Unwrapping in wrapper")
